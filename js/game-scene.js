...
class GameScene extends Phaser.Scene {
    ...
    // 分散型ウィルス（DDoS）攻撃を実行
    performDDoSAttack(targetX, targetY) {
        // 対象サーバーの確認
        const targetServer = this.getServerAt(targetX, targetY);
        if (!targetServer) {
            this.addChatMessage('ERROR', `DDoS攻撃失敗: 座標(${targetX},${targetY})にサーバーがありません`);
            return false;
        }
        
        // 敵のサーバーかチェック
        if (targetServer.owner !== 'enemy') {
            this.addChatMessage('ERROR', `DDoS攻撃失敗: 対象は敵のサーバーである必要があります`);
            return false;
        }
        
        // 自分のサーバー数をカウント
        const playerServers = this.servers.filter(server => server.owner === 'player');
        if (playerServers.length < 3) {
            this.addChatMessage('ERROR', `DDoS攻撃失敗: 少なくとも3つの制御サーバーが必要です (現在: ${playerServers.length})`);
            return false;
        }
        
        // マシンパワーチェック
        if (this.machinePower < 60) {
            this.addChatMessage('ERROR', `DDoS攻撃に必要なマシンパワーが不足しています (必要: 60, 現在: ${this.machinePower})`);
            return false;
        }
        
        // マシンパワー消費
        this.addMachinePower(-60);
        
        // DDoS攻撃実行
        this.addChatMessage('SYSTEM', `分散型ウィルス攻撃を開始しました: 対象サーバー(${targetX},${targetY})`);
        
        // 視覚エフェクト - 各サーバーから対象へのビーム
        playerServers.forEach(server => {
            const startPos = server.getPixelPosition();
            const endPos = targetServer.getPixelPosition();
            
            const beam = this.add.line(0, 0, startPos.x, startPos.y, endPos.x, endPos.y, 0xff0000, 0.6);
            beam.setLineWidth(2);
        ...
        // 対象サーバーの防御を無効化
        const oldDefenseLevel = targetServer.defenseLevel;
        targetServer.setDefenseLevel(0);
        
        // 30秒間、防御構築を無効化
        targetServer.ddosActive = true;
        
        // 視覚エフェクト - 対象サーバーの点滅
        const targetSprite = targetServer.sprite;
        this.tweens.add({
        ...
        this.time.addEvent({
            delay: 30000,
            callback: () => {
                targetServer.ddosActive = false;
                this.addChatMessage('SYSTEM', `サーバー(${targetX},${targetY})へのDDoS攻撃の効果が切れました`);
            },
            callbackScope: this,
            loop: false
        });
        
        this.addChatMessage('SYSTEM', `DDoS攻撃成功: サーバー(${targetX},${targetY})の防御レベルが${oldDefenseLevel}から0に低下し、30秒間防御構築が無効化されました`);
        return true;
    }
    
    // ボットネット作成
    createBotnet(botIds, action) {
        // ボットネットに含めるBOTの確認
        const selectedBots = [];
        for (const botId of botIds) {
            const bot = this.playerBots.find(b => b.id === botId);
            if (!bot) {
                this.addChatMessage('ERROR', `Bot${botId}が見つかりません`);
                return false;
            }
        ...
        // マシンパワーチェック
        if (this.machinePower < 50) {
            this.addChatMessage('ERROR', `ボットネット作成に必要なマシンパワーが不足しています (必要: 50, 現在: ${this.machinePower})`);
            return false;
        }
        
        // マシンパワー消費
        this.addMachinePower(-50);
        
        // ボットネット作成
        this.addChatMessage('SYSTEM', `ボットネットを作成しました: ${botIds.map(id => `Bot${id}`).join(', ')}`);
        
        // 各BOTに対してアクションを実行
        let actionSuccess = false;
        selectedBots.forEach(bot => {
            // アクションを実行
            if (bot.startAction(action, this)) {
                actionSuccess = true;
                
                // アクション完了を即時実行
                bot.actionProgress = bot.actionTime;
                bot.completeAction(this);
            }
        });
        
        // 視覚エフェクト - BOT間の接続線
        for (let i = 0; i < selectedBots.length; i++) {
            for (let j = i + 1; j < selectedBots.length; j++) {
                const bot1 = selectedBots[i];
                const bot2 = selectedBots[j];
                
                const line = this.add.line(0, 0, bot1.x, bot1.y, bot2.x, bot2.y, 0x00ffff, 0.5);
                line.setLineWidth(2);
    ...
    // ブロックチェーン防御を構築
    createBlockchainDefense() {
        // 自分のサーバー数をカウント
        const playerServers = this.servers.filter(server => server.owner === 'player');
        if (playerServers.length < 2) {
            this.addChatMessage('ERROR', `ブロックチェーン防御構築失敗: 少なくとも2つの制御サーバーが必要です (現在: ${playerServers.length})`);
            return false;
        }
        
        // マシンパワーチェック
        if (this.machinePower < 75) {
            this.addChatMessage('ERROR', `ブロックチェーン防御に必要なマシンパワーが不足しています (必要: 75, 現在: ${this.machinePower})`);
            return false;
        }
        
        // マシンパワー消費
        this.addMachinePower(-75);
        
        // ブロックチェーン防御構築
        this.hasBlockchainDefense = true;
        
        // 視覚エフェクト - サーバー間の接続線
        const lines = [];
        for (let i = 0; i < playerServers.length; i++) {
            for (let j = i + 1; j < playerServers.length; j++) {
                const server1 = playerServers[i];
                const server2 = playerServers[j];
                ...
                if (gridDist <= 3) {
                    ...
                    const line = this.add.line(0, 0, pos1.x, pos1.y, pos2.x, pos2.y, 0x0000ff, 0.4);
                    line.setLineWidth(2);
                    lines.push(line);
        ...
        // 各サーバーに防御ボーナスを適用
        playerServers.forEach(server => {
            server.hasBlockchainDefense = true;
            
            // 視覚エフェクト - サーバーの輝き
            const glow = this.add.circle(
                server.getPixelPosition().x,
                server.getPixelPosition().y,
                25,
                0x0000ff,
                0.3
            );
        ...
        this.addChatMessage('SYSTEM', `ブロックチェーン防御を構築しました: ${playerServers.length}台のサーバーが連携して防御力を共有します`);
        
        // 60秒後に効果終了
        this.time.addEvent({
            delay: 60000,
            callback: () => {
                this.hasBlockchainDefense = false;
                playerServers.forEach(server => {
                    server.hasBlockchainDefense = false;
                });
                
                // 視覚エフェクトを削除
                lines.forEach(line => line.destroy());
                
                this.addChatMessage('SYSTEM', `ブロックチェーン防御の効果が切れました`);
            },
            callbackScope: this,
            loop: false
        });
